...
async function seedWorkspaceAndExecution(
  ...
  const executionId = await ownerAuthed.mutation(async (ctx) => {
    return await createExecutionImpl(ctx as any, {
      workspaceId,
      workflowId,
      status: "running",
    });
...
describe("human-in-the-loop — approval gate", () => {
  test("approve path: respondExecutionApproval sets status to approved", async () => {
    ...
    const approvalId = await ownerAuthed.mutation(async (ctx) => {
      return await createExecutionApprovalImpl(ctx as any, {
        executionId,
        stepId: "step-approve",
        type: "approval",
    ...
    await ownerAuthed.mutation(async (ctx) => {
      return await respondExecutionApprovalImpl(ctx as any, {
        id: approvalId,
        status: "approved",
  ...
  test("idempotent: responding twice with same status returns same id", async () => {
    ...
    const r1 = await ownerAuthed.mutation(async (ctx) => {
      return await respondExecutionApprovalImpl(ctx as any, { id: approvalId, status: "approved" });
    });
...
describe("human-in-the-loop — timeout behavior", () => {
  test("auto-approve: sets approval status to approved with timedOut flag", async () => {
    ...
    await ownerAuthed.mutation(async (ctx) => {
      return await timeoutExecutionApprovalImpl(ctx as any, { id: approvalId });
    });
...
describe("human-in-the-loop — execution step state transitions (pause/resume)", () => {
  test("createExecutionApproval pauses a running step to awaiting-approval", async () => {
    ...
    const stepDocId = await ownerAuthed.mutation(async (ctx) => {
      return await createExecutionStepImpl(ctx as any, {
    ...
    const step = await t.run(async (ctx) => ctx.db.get(stepDocId as any) as any);
...
describe("human-in-the-loop — processTimedOutApprovals (batch processor)", () => {
  test("processes all pending timed-out approvals in one call", async () => {
    ...
    await ownerAuthed.mutation(async (ctx) => {
      await createExecutionApprovalImpl(ctx as any, {
    ...
    const result = await t.run(async (ctx) => {
      return await processTimedOutApprovalsImpl(ctx as any, {});
    });
  ...
  test("batch processor is idempotent — re-running does not double-process", async () => {
    ...
    const r1 = await t.run(async (ctx) => processTimedOutApprovalsImpl(ctx as any, {}));
    const r2 = await t.run(async (ctx) => processTimedOutApprovalsImpl(ctx as any, {}));
...
describe("retry orchestration — requeueStepWithRetry", () => {
  async function seedFailedStep(
    ...
    await ownerAuthed.mutation(async (ctx) => {
      return await updateExecutionStepStatusImpl(ctx as any, {
        id: stepDocId as any,
        status: "failed",
  ...
  test("re-enqueues step on first retry with exponential backoff delay", async () => {
    ...
    const result = await ownerAuthed.mutation(async (ctx) => {
      return await requeueStepWithRetryImpl(ctx as any, {
...
describe("dead-letter queue", () => {
  test("create and retrieve a dead-letter entry", async () => {
    ...
    const entryId = await ownerAuthed.mutation(async (ctx) => {
      return await createDeadLetterEntryImpl(ctx as any, {
        executionId,
    ...
    const entry = await ownerAuthed.query(async (ctx) => {
      return await getDeadLetterEntryImpl(ctx as any, { id: entryId });
    });
  ...
  test("list dead-letter entries for workspace, unresolvedOnly filter", async () => {
    ...
    const e1 = await ownerAuthed.mutation(async (ctx) => {
      return await createDeadLetterEntryImpl(ctx as any, { executionId, reason: "Error A", retryCount: 3 });
    });
    await ownerAuthed.mutation(async (ctx) => {
      return await createDeadLetterEntryImpl(ctx as any, { executionId, reason: "Error B", retryCount: 2 });
    });
    await ownerAuthed.mutation(async (ctx) => {
      return await resolveDeadLetterEntryImpl(ctx as any, { id: e1 });
    });

    const all = await ownerAuthed.query(async (ctx) => listDeadLetterEntriesImpl(ctx as any, { workspaceId }));
    expect(all.length).toBe(2);

    const unresolved = await ownerAuthed.query(async (ctx) =>
      listDeadLetterEntriesImpl(ctx as any, { workspaceId, unresolvedOnly: true })
    );
  ...
  test("resolve is idempotent", async () => {
    ...
    const entryId = await ownerAuthed.mutation(async (ctx) => {
      return await createDeadLetterEntryImpl(ctx as any, { executionId, reason: "Idempotent test", retryCount: 1 });
    });

    await ownerAuthed.mutation(async (ctx) => resolveDeadLetterEntryImpl(ctx as any, { id: entryId }));
    const r2 = await ownerAuthed.mutation(async (ctx) => resolveDeadLetterEntryImpl(ctx as any, { id: entryId }));
  ...
  test("reason is required", async () => {
    ...
    const res = await ownerAuthed.mutation(async (ctx) => {
      try {
        return await createDeadLetterEntryImpl(ctx as any, { executionId, reason: "  ", retryCount: 1 });
      } catch (err) {
        return String(err);
      }
    });
...
describe("circuit breakers — persistence", () => {
  test("starts in closed state; failure events accumulate", async () => {
    ...
    await ownerAuthed.mutation(async (ctx) => {
      return await recordCircuitEventImpl(ctx as any, {
    ...
    const cb = await ownerAuthed.query(async (ctx) => {
      return await getCircuitBreakerImpl(ctx as any, { workspaceId, key: "openrouter:completion" });
    });
  ...
  test("circuit opens after failureThreshold failures", async () => {
    ...
    const cb = await ownerAuthed.query(async (ctx) => {
      return await getCircuitBreakerImpl(ctx as any, { workspaceId, key: "slack:message" });
    });
  ...
  test("checkCircuitAllowed returns false when circuit is open", async () => {
    ...
    const result = await ownerAuthed.mutation(async (ctx) => {
      return await checkCircuitAllowedImpl(ctx as any, {
  ...
  test("success resets failure count in closed state", async () => {
    ...
    const cb = await ownerAuthed.query(async (ctx) => {
      return await getCircuitBreakerImpl(ctx as any, { workspaceId, key: "webhook:post" });
    });
  ...
  test("checkCircuitAllowed returns true for new (auto-created) key", async () => {
    ...
    const result = await ownerAuthed.mutation(async (ctx) => {
      return await checkCircuitAllowedImpl(ctx as any, { workspaceId, key: "brand-new-integration" });
    });
...
describe("circuit breaker — step gating", () => {
  test("skips step and emits step.skipped log when circuit is open", async () => {
    ...
    const gate = await ownerAuthed.mutation(async (ctx) => {
      return await gateWithCircuitBreakerImpl(ctx as any, {
...
describe("observability — step lifecycle log emission", () => {
  ...
  test("updateExecutionStatus emits execution.completed log on terminal transition", async () => {
    ...
    await ownerAuthed.mutation(async (ctx) => {
      return await updateExecutionStatusImpl(ctx as any, {
...
describe("observability — execution logs API", () => {
  test("emitExecutionLog creates a log and is queryable", async () => {
    ...
    await ownerAuthed.mutation(async (ctx) => {
      return await emitExecutionLogImpl(ctx as any, {
    ...
    const logs = await ownerAuthed.query(async (ctx) => listExecutionLogsImpl(ctx as any, { executionId }));
  ...
  test("listExecutionLogs filters by event type", async () => {
    ...
    for (const [event, msg] of [["step.started", "A"], ["step.failed", "B"], ["step.started", "C"]] as const) {
      await ownerAuthed.mutation(async (ctx) => {
        return await emitExecutionLogImpl(ctx as any, { executionId, event, level: event === "step.failed" ? "error" : "info", message: msg });
      });
    }

    const startedLogs = await ownerAuthed.query(async (ctx) => {
      return await listExecutionLogsImpl(ctx as any, { executionId, event: "step.started" });
    });
  ...
  test("event and message are required", async () => {
    ...
    const res = await ownerAuthed.mutation(async (ctx) => {
      try {
        return await emitExecutionLogImpl(ctx as any, { executionId, event: "", level: "info", message: "something" });
      } catch (err) {
        return String(err);
      }
    });
...
describe("observability — cost tracking", () => {
  test("trackStepCost rolls up to execution cost and emits a log", async () => {
    ...
    await ownerAuthed.mutation(async (ctx) => {
      return await trackStepCostImpl(ctx as any, {
    ...
    const summary = await ownerAuthed.query(async (ctx) => {
      return await getExecutionCostSummaryImpl(ctx as any, { executionId });
    });
